...
export function NewsPage() {
  ...
  const fetchNewsRef = useRef<() => Promise<void>>()
  ...
  const fetchNews = useCallback(async () => {
    ...
    const uniqueBatch = dedupeById(data)
    ...
    setItems((prev) => {
      ...
      for (const item of uniqueBatch) {
        const existingIndex = merged.findIndex((news) => news.id === item.id)
        if (existingIndex === -1) {
          merged.push(item)
        } else {
    ...
    <div className="py-4">
      ...
      <div className="row gy-4">
        {items.map((item) => {
          ...
          const firstImage = (item.pictures ?? []).find((picture) => picture.url)?.url
